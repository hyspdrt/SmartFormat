	...
	public class SourcePerformanceTests {
		...
		public void Source_Performance() {
			...
			for (var i = 0; i < 100000; i++) {
				_ = $"Address: {addr.City.ZipCode} {addr.City.Name}, {addr.City.AreaCode}\n" +
			...
			sw.Start();
			for (var i = 0; i < 100000; i++) {
				_ = formatter.Format(format, addr);
			}
			sw.Stop();
			...
			for (var i = 0; i < 100000; i++) {
				_ = formatter.Format(format, dict);
			}
			sw.Stop();
			...
			for (var i = 0; i < 100000; i++) {
				_ = formatter.Format(format, jObject);
			}
			sw.Stop();
