...
namespace SmartFormat.Core.Parsing {
	...
	public abstract class FormatItem {
		...
		protected FormatItem(SmartSettings smartSettings, FormatItem parent, int startIndex) :
			this(smartSettings, parent.baseString, startIndex, parent.baseString.Length) {
		...
		/// </summary>
		public string RawText => baseString[startIndex..endIndex];

		public override string ToString() {
			return endIndex <= startIndex
				? $"Empty ({baseString.Substring(startIndex)})"
				: $"{baseString[startIndex..endIndex]}";
		}
