	...
	public class Parser {
		...
		public Format ParseFormat(string format, string[] formatterExtensionNames) {
			...
			for (int i = 0, length = format.Length; i < length; i++) {
				...
				if (currentPlaceholder == null) {
					if (c == openingBrace) {
						...
						if (c == '(' || c == '.') {
						...
						} else if (c == ')' || c == ':' || format[i + 1] == closingBrace) {

							var hasClosingBrace = false;

							if (c == ')' || (format[i + 1] == closingBrace && c != ':')) {
							...
							if (namedFormatterOptionsStartIndex == -1) {
								...
								var formatterName = format[namedFormatterStartIndex..i];
								...
								var formatterName = format[namedFormatterStartIndex..namedFormatterOptionsStartIndex];
								...
								if (FormatterNameExists(formatterName, formatterExtensionNames)) {
									...
									parentPlaceholder.FormatterOptions = format.Substring(
										namedFormatterOptionsStartIndex + 1,
										namedFormatterOptionsEndIndex -
											(namedFormatterOptionsStartIndex + (hasClosingBrace ? 1 : 0))
									);
			...
			if (current.parent != null || currentPlaceholder != null) {
				parsingErrors.AddIssue(parsingErrorText[ParsingError.MissingClosingBrace], format.Length, format.Length);
