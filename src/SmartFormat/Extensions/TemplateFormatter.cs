		...
		public bool TryEvaluateFormat(IFormattingInfo formattingInfo) {
			...
			if (!_templates.TryGetValue(templateName, out var template)) {
				if (Names.Contains(formattingInfo.Placeholder.FormatterName)) {
					var fname = formattingInfo.Placeholder.FormatterName;
					var fexp = $"Formatter '{fname}' did not find a registered template named '{templateName}'";
					throw new FormatException(fexp);
				}
				return false;
			}
